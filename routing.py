...
def _raise_for_ors_error(resp):
    if resp.status_code == 401:
        raise RoutingError("OpenRouteService rejected the API key (401 Unauthorized).")
    if resp.status_code == 403:
        raise RoutingError("OpenRouteService API key lacks permission for this request (403).")
...
def get_fast_and_safe_routes(profile_key, start, end, incidents=None):
    ...
    entry from risk.score_route(). Combines three sources of candidate
    ...
    scored = [
        {**route, "risk": risk.score_route(route["coordinates"], incidents)}
        for route in routes
    ]
