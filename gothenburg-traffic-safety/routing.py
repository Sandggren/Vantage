...
def reverse_geocode(lat, lon):
    """Nearest street address for a point, e.g. "Vasagatan 12, Gothenburg,
    VG, Sweden", or None if nothing is found. Limited to addresses and
    streets -- otherwise the nearest shop or café name comes back."""
    _require_api_key()
    params = {
        "api_key": config.OPENROUTESERVICE_API_KEY,
        "point.lat": lat,
    ...
    resp = requests.get(f"{config.ORS_BASE_URL}/geocode/reverse", params=params, timeout=10)
    _raise_for_ors_error(resp)
    features = resp.json().get("features", [])
    return features[0]["properties"].get("label") if features else None
