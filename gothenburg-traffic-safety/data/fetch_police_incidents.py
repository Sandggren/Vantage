...
# `title` is the police's own event name, e.g. "21 september 15.38,
# Trafikolycka, Göteborg" -- its leading date/time is when the accident
# happened, whereas `datetime` is when the report was published. `url` links
# to the full report on polisen.se.
FIELDNAMES = ["id", "datetime", "title", "street", "lat", "lon", "severity", "road_users", "summary", "url", "source"]
POLICE_SITE_URL = "https://polisen.se"
...
def main():
    ...
    for event in events:
        ...
        rows.append({
            ...
            "summary": summary,
            "url": POLICE_SITE_URL + event["url"] if event.get("url") else "",
            "source": "POLICE_API",
        })
