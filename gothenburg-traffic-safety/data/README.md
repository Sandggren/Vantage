...
- (on the dashboard) a 🟣 in the accident details panel, and a separate
  hotspot layer ("More accident hotspots", off by default) — in the same
  warm colors as the real one, so only the dots are purple

There's no text label on them — purple is the marker, and a line under
each map explains it.
