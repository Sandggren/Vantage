...
`app.py` and `pages/2_Route_Check.py`), synthetic markers get:
...
`police_incidents_gothenburg.csv` additionally has `datetime`, `street`,
and `summary`; the synthetic file has `near` (a landmark name) instead of
