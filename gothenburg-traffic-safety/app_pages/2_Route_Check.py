...
st.set_page_config(page_title="Vantage · Route Risk Check", page_icon=config.LOGO_ICON_PATH, layout="wide")

st.title("🧭 Route Risk Check")
st.caption(
    "Compare a fast route against a lower-risk route between two points, "
    "each scored on reported incidents near the road and on how busy the "
    "roads it uses are (Trafikverket traffic volumes)."
)

if config.OPENROUTESERVICE_API_KEY in ("", "your_key_here"):
    st.warning(
...
with st.expander("🔑 API key check"):
    # Compare this line between computers when routing works on one but
    # not the other -- it shows the key's fingerprint, never the key.
    st.caption(routing.key_summary())
...
if st.button("Find routes", type="primary", disabled=not ready):
    ...
    try:
        ...
        st.session_state.route_mode = profile_key
    except routing.RoutingError as exc:
        st.error(str(exc))
...
def show_traffic_summary(route_risk, mode):
    """Caption lines on how busy the roads on a route are, and how much of
    the risk score comes from traffic vs. incidents."""
    traffic = route_risk.get("traffic")
    if traffic is None:
        st.caption("🚗 Traffic volume not included (Trafikverket data unavailable).")
        return
    # Bike/foot paths right beside a road can't be told apart from the road
    # itself in this data, so don't claim a cyclist rides *on* the E6.
    relation = "on" if mode == "Car" else "along"
    if traffic["busiest"]:
        busiest = traffic["busiest"]
        st.caption(
            f"🚗 {traffic['busy_km']:.1f} km {relation} busy roads "
            f"({config.BUSY_ROAD_VEHICLES_PER_DAY:,}+ vehicles/day) · busiest: "
            f"{busiest['road']}, {busiest['vehicles_per_day']:,} vehicles/day"
        )
    else:
        st.caption(
            f"🚗 Doesn't run {relation} any roads in Trafikverket's traffic data "
            "(state roads) — no traffic volume added."
        )
    st.caption(
        f"Risk points/km: {route_risk['incident_score_per_km']:.2f} from incidents + "
        f"{route_risk['traffic_score_per_km']:.2f} from traffic"
    )
...
if fast and safe:
    ...
    route_mode = st.session_state.get("route_mode", "Car")
    ...
    with col_fast:
        ...
        show_traffic_summary(fast["risk"], route_mode)
    ...
    with col_safe:
        ...
        show_traffic_summary(safe["risk"], route_mode)
    ...
    # Busy roads the routes use, in the same red scale as the home map,
    # drawn wide underneath the route lines.
    used_roads = set()
    for route in (fast, safe):
        if route["risk"].get("traffic"):
            used_roads.update(route["risk"]["traffic"]["feature_indices"])
    if used_roads:
        try:
            flows = traffic_flow.load_vehicle_flows()
            folium.GeoJson(
                {"type": "FeatureCollection", "features": [flows["features"][i] for i in sorted(used_roads)]},
                style_function=lambda f: {
                    **traffic_flow.vehicle_flow_style(f),
                    "weight": traffic_flow.vehicle_flow_style(f)["weight"] + 6,
                    "opacity": 0.6,
                },
                tooltip=folium.GeoJsonTooltip(
                    fields=["AADT_TOT", "VAEG_NUM"],
                    aliases=["Vehicles/day:", "Road:"],
                    localize=True,
                ),
            ).add_to(m)
        except Exception:
            pass  # purely visual; the scores above already include traffic
